...
// DoCompression()
// 
// Purpose: Calls Compression Routines
// 
// Author : [Josh Whelchel], Created : [9/28/2002 3:38:58 PM]
// Parameters:	
// 
// [*cFile] :
// [*cTempFile] :
 ...
 */
	...
	if (bCompress)
	{
		r = lzo1x_1_compress(bInput,in_len,out,&out_len,wrkmem);
		if (r != LZO_E_OK)
		{
			free(out);
			return 2;
	...
	{
/*
 * Step 4: decompress, now going from `out' to `in'
 */
		r = lzo1x_decompress(bInput,in_len,out,&out_len,wrkmem);
		if (r != LZO_E_OK)
		{
			free(out);
			return 1;
	...
	out = (lzo_byte*)realloc(out,out_len);
